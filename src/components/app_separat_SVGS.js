...
const ResultsChart = ({score}) => {
    const x = i => 20 * i;
    const rectHeight = 20, tickHeight = 6;

    return <svg width={900} height={5 + 20 + 20 + rectHeight}>

        <text x={10} y={15} text-anchor="start">Risk</text>

        {/* Label above risk bars */}
        <g transform={"translate(60,5)"}>
            {categories.map(c => <text x={(x(c.min) + x(c.max)) / 2} y={10}
...
const Histogram = ({score}) => {
    ...
    return <svg width="500" height={maxBarHeight}>
...
const Survival = ({score}) => {
    ...
    const y = scaleLinear().range([0, 200]).domain([100, 0]);
    const x = i => 10 * i;
        ...
        return (
            <div id="app">
                ...
                <p>It is designed to be easy-to-use, and require only parameters that are commonly available at hospital
                    presentation.</p>
                <p>For full technical details, see <b>the paper</b>.</p>
                ...
                {(score !== null) ? <>
                        <p>Score is <span style={{fontSize: "2em"}}><b>{score}</b>/21</span> (larger is worse).
                        </p>
                        <ResultsChart score={score}/>
                        <Histogram score={score}/>
                        {/* <Survival score={score} /> */}
                    </> :
                    <p>Please select an option for every variable to calculate a mortality score.</p>
                }
